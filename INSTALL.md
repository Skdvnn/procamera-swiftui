...
**Build 78 phone test pack (fullscreen histogram polish):**
[`ProCamera-unsigned.ipa`](../../releases/download/latest-build/ProCamera-unsigned.ipa)
from [`latest-build`](../../releases/tag/latest-build).

**Preferred:** `git pull` → Xcode **Cmd+R** → confirm build **78**.
Fullscreen fade is lighter; histogram glass sits higher above the shutter and is
taller, with a machined rim, recessed keyline, exposure grid, and roomier ISO/S.
